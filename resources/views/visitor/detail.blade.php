...
<nav aria-label="breadcrumb" class="mb-4">
    <ol class="breadcrumb m-0" style="font-size: 13px;">
        ...
        <li class="breadcrumb-item">
            <a href="{{ route('visitor.home', ['category' => $article->kategori->nama_kategori]) }}" class="text-decoration-none fw-bold" style="color: #2e7d32;">{{ $article->kategori->nama_kategori }}</a>
        </li>
        <li class="breadcrumb-item active text-muted" aria-current="page">{{ Str::limit($article->judul, 50) }}</li>
    </ol>
</nav>
...
<div class="row">
    <div class="col-lg-8 col-12 mb-4">
        <div class="card border-0 shadow-sm" style="border-radius: 12px; overflow: hidden; background: #ffffff;">
            <img src="{{ asset('storage/gambar/' . $article->gambar) }}" class="img-fluid w-100" style="height: 360px; object-fit: cover;" alt="Banner Gambar">
            ...
            <div class="card-body p-4">
                ...
                <h1 class="fw-bold mb-3" style="color: #212529; font-size: 28px; line-height: 1.3;">{{ $article->judul }}</h1>
                ...
                <div class="d-flex align-items-center gap-2 mb-4 pb-3 border-bottom">
                    <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold text-white shadow-sm" 
                        ...
                        {{ strtoupper(substr($article->penulis->nama_depan, 0, 1)) }}{{ strtoupper(substr($article->penulis->nama_belakang, 0, 1)) }}
                    </div>
                    <div>
                        <p class="m-0 fw-semibold text-dark" style="font-size: 13.5px;">
                            {{ $article->penulis->nama_depan }} {{ $article->penulis->nama_belakang }}
                        </p>
                        <p class="m-0 text-muted" style="font-size: 11.5px;">{{ $article->hari_tanggal }} WIB</p>
                    </div>
                </div>
            ...
            <div class="d-flex flex-column gap-3">
                @foreach($relatedArticles as $related)
                    <div class="d-flex align-items-start gap-3">
                        <img src="{{ asset('storage/gambar/' . $related->gambar) }}" class="object-fit-cover shadow-sm" style="width: 64px; height: 64px; border-radius: 6px; flex-shrink: 0;" alt="Thumbnail">
                        <div>
                            <a href="{{ route('visitor.article.show', $related->id) }}" class="text-dark fw-medium text-decoration-none d-block lh-sm mb-1 link-hover-effect" style="font-size: 13px;">
                                {{ $related->judul }}
                            </a>
                            <span class="text-muted d-block" style="font-size: 10.5px;">{{ $related->hari_tanggal }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
